...
#show: sdu-thesis.with(
  ...
  // 中文摘要一般 300 - 800 个汉字
  abstract_zh: [
    异步拜占庭容错（Byzantine Fault Tolerance, BFT）共识协议能够在极端的网络延迟下保证系统活性。当前主流基于异步公共子集（Asynchronous Common Subset, ACS）的协议（如 Dumbo 系列）普遍采用广播与共识严格串行的两阶段架构。然而，这种串行设计导致每轮共识中“诚实但迟到”的广播数据被直接丢弃，造成了网络带宽浪费与系统吞吐量损失。

    为了解决这一带宽浪费问题，本文提出了一种面向串行 ACS 协议的跨轮次的广播数据复用机制。该机制不需要修改原有 ACS 的内部决定语义。具体而言，节点在每轮结束后会进入一个有限时长的宽限窗口（Grace Window），用于收集迟到但已生成可用性证明的广播数据，并将其存入本地复用池；在后续轮次中，这些历史数据会被编码为轻量引用，作为新提案的一部分重新注入共识系统。同时，为了应对引用数据在本地未命中的情况，本文设计了独立的异步拉取（Fetch）路径来保障协议活性。
  ...
  header-rows: 2,
    ...
    @tbl_grace_sensitivity_clean 记录了在本地多进程环境下，调整宽限窗口时间（Grace Window）对系统吞吐量及复用条目数回收效率的影响。
    ...
    #thesis-table(
      ...
      caption: "Dumbo 路径上 QUIC宽限窗口敏感性试验结果",
...
1. *机制设计与安全性论证*：提出了一种面向串行 ACS 协议的跨轮次复用机制。将复用对象严格限定为“已取得可用性证明、但因时序落后未被采纳”的历史提案，通过引入生命周期宽限窗口，确保复用机制作为宿主层逻辑不侵入 ACS 的决定语义，并在理论上维护了原协议的安全性与活性边界。
2. *原型系统实现*：围绕统一提案格式编码、缓存生命周期管理、引用消费去重以及基于异步拉取（Fetch）的缺失数据恢复等关键环节，对协议栈进行了端到端的设计与改造，并基于 Rust 语言完成了融合多语言 ACS 后端的原型系统开发。
...
基于有向无环图（Directed Acyclic Graph, DAG）的异步 BFT 协议绕开了 ACS，如 DAG-rider @keidarAllYouNeed2021；2022 年的 Narwhal and Tusk @danezisNarwhalTuskDAGbased2022 通过将 Mempool 协议和共识协议进行并行解耦，获得了很好的性能。2025 年的 Mysticeti @babelMysticetiReachingLatency2025 则将 DAG 路径的延迟推至消息复杂度下界。基于 DAG 的协议实现相比基于 ACS 的协议在实现上会更复杂。

国内方面，异步 BFT 方向也已出现一些面向具体问题的改进工作。例如，文献@WangYaoQiGuaYongYuQuKuaiLianDeGaoXiaoYiBuBaiZhanTingRongCuoSuanFa2023 提出 PenguinBFT，主要针对异步共识中的通信与交易恢复开销进行优化；文献@ZhouYiKeIDumboQuanLiuChengYinSiBaoHuYiBuBaiZhanTingGongShiXieYi2025 提出 IDumbo，在 DumboBFT 框架下进一步融入全流程隐私保护机制。
...
第一种思路是弱化广播原语，在广播阶段不强制要求数据全网可达，而是在共识决定输出后，增加一个异步的拉取（Retrieval）阶段来下载缺失数据。DispersedLedger 和 sDumbo @guoSpeedingDumboPushing2022 都使用了类似的优化方法。

第二种思路是将广播与共识完全解耦，通过流水线（Pipelining）设计同时推进多轮共识。Dumbo-NG @gaoDumboNGFastAsynchronous2022 通过维护多轮次的水位线（Watermark）来实现这一目标。JUMBO @chengMathsfJUMBOJUMBOFully2025 则在 Dumbo-NG 框架的基础上，通过签名聚合技术将通信复杂度从 $O(n^3)$ 降至 $O(n^2)$，为此类并行化方案的后续扩展提供了重要的工程参考。
...
在严格串行的 ACS 架构中，系统推进必须满足活性要求：只要节点收集到 $N-f$ 个合法的广播证明，就必须立即进入共识阶段，而不能无限期等待所有 $N$ 个提案完成（防止拜占庭节点因拒不发包而导致系统停滞）。这一机制带来一个副作用：本轮共识中，剩下至多 $f$ 个诚实节点的广播提案即使最终到达，也会因时序落后而被系统丢弃，造成严重的带宽浪费。并行的 ACS 协议没有这种浪费问题。

DispersedLedger @yangDispersedLedgerHighthroughputByzantine2022 对此问题有过深入探讨。在基于 BKR94 范式的 HoneyBadger 中，节点一旦收到 $N-f$ 个 RBC 完成消息，便会向本地所有的 $N$ 个 ABA 实例投出初始票。在网络同质性较好的情况下，其余 $f$ 个 RBC 仍有较大概率在 ABA 结束前完成全局终止。然而，在网络波动较大的情况下，浪费依然严重。为此，文献 @yangDispersedLedgerHighthroughputByzantine2022 提出了 Inter-node Linking 机制：在消息头中维护向量与水位线（本质上是提取并维护一种轻量级的偏序信息），使后续节点能够据此计算出历史消息的合法性并将其链入主账本。

需要指出的是，Inter-node Linking 机制深度修改了 BKR94 范式内部协议，其耦合度极高。对于基于 CKPS01 范式（即 PRBC + MVBA）的 Dumbo 风格协议而言，这种深度的内部结构修改难以直接迁移。针对这一痛点，本文提出一种非侵入式的跨轮次复用机制，旨在无需引入复杂偏序状态机、不改动 MVBA 内部语义的前提下，解决串行 ACS 中的广播浪费问题。
...
在该模型下，我们旨在构造一个 $N$ 节点参与的异步原子广播协议。协议按连续的轮次（Epoch）推进，每轮结束后系统输出一批新的交易并追加到全局已提交日志中。该协议需以压倒性概率满足以下性质：
...
我们采用与 HoneyBadgerBFT @millerHoneyBadgerBFT2016 类似的构造思路，即先实现异步公共子集，再结合门限加密将 ACS 转化为原子广播。该 ACS 协议需满足以下性质：
...
因此，跨轮次复用的核心前提是：目标对象的广播子协议必须能够产出公开可验证的证据（例如阈值签名或完成证书）。满足这一要求的典型协议是 PRBC @guoDumboFasterAsynchronous2020。PRBC 完成时生成的证书，使节点无需重新执行广播交互即可确认该条目的可用性。
...
1. *已缓存（Cached）*：条目在规定的宽限窗口（Grace Window, $G$）时间内完成 PRBC 并生成证明，进入复用池；
...
#thesis-figure(
  ...
  #cetz.canvas({
    ...
    content((-2.5, 0.6), text(font: ("Times New Roman", "SimSun"), size: 9pt)[宽限窗口内完成 PRBC])
...
为了实现跨轮次复用机制与底层共识引擎的解耦，本文在架构设计上抽象出了一层事件驱动边界。我们将 ACS 核心协议视为一个内部执行严格串行逻辑、但向外异步抛出状态事件的“黑盒”状态机；而外层的宿主驱动程序则通过订阅并监听这些事件来推进跨轮复用与数据拉取逻辑。
...
#algox(
  ...
  [$quad$ #emph[ReusePool]: Dictionary mapping $id$ to $("payload", "proof", "state", "epoch")$],

  [$quad$ #emph[GraceWindow]: Time threshold $G$],

  [$quad$ #emph[MaxReuse]: Limit parameter $K$],

  [],

  [*On Event:* #emph[PRBC_Complete] (node $i$, payload $p$, proof, $pi$)],
  [
    1. *If* system is currently within #emph[GraceWindow] for Epoch $e$:
  ],

  [
    2. $quad$
      #emph[ReusePool] .add($id(p)$, ($p$, $pi$, "Proposable", $e$))
  ],

  [],

  [*On Event:* #emph[ACS_Decide] (Epoch $e$, DecisionSet $D$)],

  [
    1. Start #emph[GraceWindow] timer
      of duration $G$
  ],

  [
    2. Wait for #emph[GraceWindow]
      to expire
  ],
  ...
  [
    6. $quad quad$ *If* $id$ not in #emph[ReusePool]:
  ],

  [
    7. $quad quad quad$
      $"payload" <- "Asynchronous"\_"Fetch"(
        "Source" = i,
        "hash" = h
      )$
  ],
  ...
  [
    8. $quad quad quad$
      *If* timeout or Verify($pi$, payload) = False:
  ],

  [
    9. $quad quad quad quad$
      Reject and drop $upright("ref")$
  ],

  [
    10. $quad quad$
      #emph[ReusePool] [id].state
      $arrow.l$ "Consumed"
  ],
  ...
  [13. #emph[Trigger_Epoch_Start] ($e+1$)],

  [],

  [*On Event:* #emph[Trigger_Epoch_Start] (Epoch $e+1$)],

  [
    1. Evict entries in #emph[ReusePool] where $e+1 - upright("entry.epoch") > upright("MaxTTL")$
  ],

  [
    2. selected_refs $arrow.l$
      Select up to $K$ "Proposable" items from #emph[ReusePool]
  ],

  [
    3. new_txs $arrow.l$ Fetch new transactions from Mempool
  ],

  [
    4. Start ACS($e+1$) with input
      $chevron.l
      upright("new_txs"),
      upright("selected_refs")
      chevron.r$
  ],
)
...
1. *广播截留（步骤 1-3）*：在 Epoch $e$ 的 ACS 决定输出后，在时长为 $G$ 的宽限窗口内，将诚实但迟到的 PRBC 结果连同完成证明写入本地复用池。
2. *跨轮提案构造（步骤 14及 Start 事件）*：新一轮启动时，从复用池提取至多 $K$ 个未消费的历史引用，与新交易打包成统一提案作为 ACS 协议的输入。
3. *引用解析与拉取（步骤 6-10）*：当节点收到包含历史引用的共识输出时，首先查询本地池；若未命中则根据来源信息触发异步拉取。只有通过 $pi$ 验证的原始负载才会被视为有效输入。
...
1. *宽限窗口的延迟权衡*：宽限窗口 $G$ 会在轮次切换间隙引入强制等待。该参数需根据网络延迟方差动态调优，以少量时延换取大幅吞吐增益。由于 $G$ 是确定性的有限常量，其不会破坏原协议的概率终止性（Probabilistic Termination）。
2. *拉取（Fetch）的异步容错*：若全网因极端情况（如发送者掉线）无法恢复原始负载，同步阻塞将导致活性丧失。为此，本机制的拉取路径被设计为带有超时的异步任务（如算法1第8-10行）。超时即放弃该引用，这种“尽力而为”的策略阻断了因局部历史数据缺失而导致全局协议停滞的风险。
...
3. *驱动层与运行时*：负责节点生命周期管理、网络 I/O 收发调度、提案构造、复用池内存维护、异步拉取逻辑触发以及实验指标的采样汇总。

在具体工程设计上，系统遵循第三章提出的事件驱动边界模型。ACS 模块被封装为通过异步通道（Async Channels）通信的独立共识黑盒；驱动层通过非阻塞监听 ACS 抛出的 #emph[Send]、#emph[Broadcast] 等物理网络请求，以及 #emph[PRBC_Complete]、#emph[ACS_Decide] 等逻辑状态事件，完成相应的网络转发与数据展开处理。复用池则实现为一个带有 TTL（生存时间）限制与容量阈值的并发安全哈希表。

系统的整体架构如 @fig_arch 所示。每一轮次中，提案构造模块从交易池和本地复用池中分别取新交易与历史引用，按第三章所述统一提案格式构造本轮输入，再由驱动层将其注入 ACS 以启动新一轮共识。共识执行过程中，ACS 向外层抛出两类异步事件：#emph[PRBC_Complete]（PRBC 完成并产出可用性证明的提案）将被存入提案存储（ProposalStore）；#emph[ACS_Decide]（MVBA 达成的全局决定集合）则触发区块组装流程——驱动层从 ProposalStore 取出被选中的提案，送入区块组装模块。

当驱动层监听到 #emph[ACS_Decide] 事件后，将依次执行以下操作：

- *解析引用*：遍历决定集合中的引用标识，查询本地复用池。
- *状态更新*：命中池中的条目被标记为已消费，以防在后续 epoch 中被重复提案。
- *异步回收*：启动后台协程，在宽限窗口结束后自动清理过期或溢出的条目，防止内存开销随运行轮数无限累积。

区块组装模块识别提案中的历史引用部分后，首先查询本地复用池以获取原始数据；若本地未命中，则调用拉取模块，向来源节点发起异步拉取。拉取机制采用"按需触发"设计：当驱动层在 #emph[ACS_Decide] 结果中发现某条目在本地缺失时，立即根据引用携带的来源信息发起定向拉取。整个过程发生在共识内核之外，不阻塞 ACS 后续轮次的开启，也不影响共识链路的整体推进。

所有提案数据（新交易与解析完成的引用数据）经 TPKE 解密后拼装为最终区块，提交给状态机。本轮结束后，提案存储中未被选中的提案（若其 PRBC 在宽限窗口内完成）由驱动层缓存至复用池，供后续轮次引用。
...
实验统一配置参数为：节点总数 $N=12$，最大容错数 $f=3$。对于跨轮次复用机制，其参数固定为：单轮重提上限 4，条目过期轮数 10，内存池容量根据 Batch Size 规模 $b$，在 $b <= 16384$ 时为 4096，$b >= 24576$ 时提升至 524288。每组实验运行 4 至 6 轮共识并重复执行 2 至 3 次，取平均值作为最终统计结果。

为了从多维度评估复用机制的性能，本文使用以下四项指标：

1. *端到端吞吐量（TPS_wall）*：按实际物理时间（Wall-clock Time）累计计算的交易交付效率。反映用户视角的端到端系统效率。
2. *追踪通信成本（bytes/tx）*：每成功完成一笔交易，驱动层需要发送的总通信字节数。其计算公式为：

$
  upright("bytes/tx") = frac(upright("send_payload") + upright("proposal_payload") + upright("proposal_proof"), upright("total_delivered_tx"))
$

其中，send_payload 为驱动层发送的原始负载，proposal_payload 为共识提案负载，proposal_proof 为针对提案负载的可用性证明，total_delivered_tx 为系统最终成功交付的交易总数。采用该归一化指标而非总字节数，能够精确衡量复用机制在交易量增长时对通信效率的真实影响。
...
本测试通过三类扰动验证复用机制在非理想条件下的鲁棒性：固定传输延迟（每条消息附加 10/25/50 ms 固定额外延迟）、随机传输抖动（每帧消息随机附加 $0 tilde J$ ms 均匀抖动）和慢诚实节点（额外延迟 80/150/300 ms）。实验采用 FIN 后端，$N=12$，$b=1024$，每组运行 6 轮共识并重复 3 次，结果如 @tbl_network_faults 所示。
...
慢诚实节点场景揭示了 ACS 机制的一个内在特性。在复用关闭配置下，单节点以额外 80 ms 延迟发送时，系统绝对吞吐（44,559 TPS）反而高于无慢节点基线（42,249 TPS）。ACS 仅需收集 $N-f=9$ 个最快的 PRBC 完成信号即可决策，慢节点被主动排除，其延迟不仅不拖累共识进度，还因减少了 TPKE 解密数量而带来轻微提速。这一效应在慢节点延迟达到 300 ms 时仍然成立。在复用开启条件下，慢节点的迟到数据被宽限窗口截留并在后续轮次中回收，系统吞吐在 80 ms 至 300 ms 的延迟范围内稳定保持 $+2.3%$ 至 $+8.9%$ 的增益。
...
#thesis-table(
  ...
  caption: "不同慢诚实节点数量下的吞吐量与复用增益（b=1024, N=12, 额外延迟=150ms）",
  label-name: "tbl_slow_count_sweep",
)

数据在慢节点数 $= 4$ 处出现明显跳变，原因在于 $N=12$、$f=3$ 配置下 ACS 每轮需收集 $N-f=9$ 个 PRBC 完成信号。慢节点数 $<= 3$ 时，系统仍有 9 个以上的快速诚实节点，ACS 可以完全绕过所有慢节点直接推进共识。此时慢节点的迟到提案虽会被宽限窗口截留，但 ACS 每轮恰好从 9 个最快者中选取提案，复用池的引用与去重开销无处消化，复用增益为 $-6.9%$ 至 $+1.1%$。

当慢节点数增至 4 时，快速诚实节点降至 8 个，ACS 被迫等待至少 1 个 150 ms 慢节点，绝对吞吐从 43,408 TPS 骤降至 9,187 TPS（$-78.8%$）。与此同时，每轮至多 $f=3$ 个诚实提案因时序落后被排除在决策集合之外，宽限窗口从"无物可收"转为"每轮回收至多 3 个迟到提案"，复用增益在同一边界点跳升至 $+30.0%$，为全部传输扰动实验中的最高值。此后慢节点数继续增加，复用增益稳定在约 $+25.8%$，但绝对吞吐持续恶化。
...
为评估复用机制在存在恶意节点时的鲁棒性，本文测试了两类拜占庭行为：静默节点（Silent Node）和无效拉取响应（Invalid Fetch Response）。实验配置为 $N=12$、$b=4096$、$f=1$（单拜占庭节点），运行 8 轮共识，结果如 @tbl_byzantine 所示。
...
两类拜占庭行为下的复用增益（$+15.9%$ 至 $+20.0%$）均与干净网络下的水平（$+23.3%$）基本一致，表明单点恶意行为未能实质性削弱复用机制的回收效率。

两类场景的绝对吞吐量差异值得关注：无效拉取场景（复用开启时 135,582 TPS）反而高于静默节点场景（128,375 TPS）。无效拉取场景中，拜占庭节点仍正常参与 ACS 共识和提案广播，仅在被要求提供缺失数据时返回损坏响应；而静默节点完全不提供提案，使 ACS 每轮可用的诚实提案池缩小，总交付量随之下降。这一差异说明，"提案参与度"对吞吐量的影响比"数据完整性攻击"更为直接——前者减少了 ACS 的输入规模，后者仅增加了拉取路径的重试成本。

== 宽限窗口敏感性测试

本节分析宽限窗口时长对复用收益的影响。理想网络下的完整吞吐量随宽限窗口时长变化数据（0 ms 至 800 ms，波动在 $plus.minus 4%$ 以内）见附录 @tbl_grace_sensitivity_clean；正文聚焦更能体现宽限窗口价值的边界故障条件。
...
仅含 2 个慢诚实节点时，系统仍有 10 个快速诚实节点，ACS 选取前 9 个完成者即可推进，150 ms 慢节点始终赶不上决策节奏。宽限窗口的长短无法改变这一基本时序关系，增益贡献有限（最高 $+2.8%$ at $g=50$）。

引入第 3 个故障节点（静默拜占庭）后，诚实节点池降至恰好 9 个，ACS 须等待全部 2 个慢节点完成 PRBC 方能凑齐提案。此时，宽限窗口开始产生可观测的收益：当 $g=800$ ms 时，复用开启下的吞吐量（125,789 TPS）比复用关闭高出 $+10.4%$；而在 2 慢无拜占庭的对照组，同一窗口长度下两种配置已无明显差距。
...
上述结果表明，宽限窗口的参数选择与部署环境密切相关：在网络同质、节点健康的局域网环境中，短窗口已足够；当系统运行在接近容错极限、存在慢节点或拜占庭节点的场景下，适当延长宽限窗口能够有效回收因被迫等待而产生的时序落后广播，提供额外的吞吐保障。
...
本研究存在以下局限。所有实验均基于本地多进程 QUIC 回环网络，缺乏真实广域网环境下的跨机器验证；WAN 中各节点间延迟的异质性与长尾效应可能进一步放大复用收益，也可能对拉取路径的触发频率产生不可预知的影响。拜占庭攻击仅覆盖静默节点和无效拉取响应两种基础行为，选择性静默、定时欺诈、多轮共谋等针对性策略尚未测试。节点规模受单机资源限制，仅测试至 $N=16$，$N >= 20$ 的表现有待后续研究。此外，追踪通信成本依赖驱动层应用数据包统计，未来应通过网卡级抓包（tcpdump 或 eBPF）获取更精确的物理网络层带宽数据。

= 总结与展望

// 约 1000 字

本文针对基于 ACS 的 BFT 共识协议中，因广播与共识阶段严格串行而导致“诚实但迟到”的广播数据被丢弃的问题，设计并实现了一种跨轮次广播复用机制。

在机制设计与工程实现方面，本课题在不修改底层 ACS 协议一致性内核的前提下，构建了一套完整的缓存复用流程。该机制通过引入有限时长的宽限窗口截留历史提案，采用“内联负载+引用”的统一提案编码，并结合复用池的生命周期管理以及异步拉取机制，实现了对迟到广播成果的安全回收。
