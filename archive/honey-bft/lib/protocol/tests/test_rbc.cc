...
namespace {
    ...
    struct TransportMock {
        ...
        Task<void> unicast(int target, const RBCMessage& msg)
        {
            unicasts.push_back({ target, msg });
            co_return;
        }

        Task<void> broadcast(const RBCMessage& msg)
        {
    ...
    struct CryptoMock {
        ...
        Task<MerkleTreeType> async_build_merkle_tree(int K, int N, BytesSpan data)
        {
            std::vector<std::vector<Byte>> shards;
            shards.reserve(N);
            for (int i = 0; i < N; ++i) {
        ...
        Task<std::optional<std::vector<Byte>>> async_decode(
            int /*K*/, int /*N*/,
            std::span<const std::pair<int, std::vector<Byte>>> received_shards)
        {
            if (received_shards.empty()) {
                co_return std::nullopt;
            }

            co_return received_shards[0].second;
        }

        // ---------------------------------------------------------------------
        // Concept: CanExtractPayload
        // ---------------------------------------------------------------------
        ValPayload extract_val_payload(const MockMerkleTree& tree, int node_id)
        {
            return ValPayload {
                .root_hash = tree.root(),
    ...
    struct VectorStream {
        ...
        Task<std::optional<RBCMessage>> next()
        {
            if (idx >= msgs.size())
                co_return std::nullopt;
            co_return msgs[idx++];
...
TEST_CASE_FIXTURE(ReliableBroadcastTest, "ReliableBroadcastTest.DeliversWithOnlyReadys")
{
    ...
    // Inject one ECHO to ensure we have enough stripes (k=2) for decoding
    stream.msgs.push_back(make_echo(2));
