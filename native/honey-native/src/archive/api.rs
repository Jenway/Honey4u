...
pub(crate) fn encode_result<T>(value: &T) -> Result<Vec<u8>, String>
where
    T: for<'a> Serialize<
...
pub(crate) fn decode_result<T>(payload: &[u8]) -> Result<T, String>
where
    T: Archive,
...
where
    ...
    >,
{
    encode_result(value).map_err(PyValueError::new_err)
}
...
where
    ...
    for<'a> <T as Archive>::Archived: rkyv::bytecheck::CheckBytes<rkyv::api::high::HighValidator<'a, Error>>
        + Deserialize<T, rkyv::api::high::HighDeserializer<Error>>,
{
    decode_result(payload).map_err(PyValueError::new_err)
}
