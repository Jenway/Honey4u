...
from honey.infra.exceptions import UnknownTagError
from honey.protocol.messages import Channel, InboundEnvelope, ProtocolEnvelope, ProtocolMessage
        ...
        *,
        ...
        while True:
            try:
                msg = await queue.get()
                if broadcast:
                    payload = cast(ProtocolMessage, msg)
                    ...
                    recipient, payload = cast(PointToPointOutbound, msg)
                    await self.transport.send(
                        recipient,
                        ProtocolEnvelope(
                            round_id=self.round_id,
