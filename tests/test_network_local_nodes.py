...
def _assert_embedded_runtime_results(results: list[Any]) -> None:
    assert len(results) == 4
    assert all(result.rounds == 1 for result in results)
...
def test_local_honeybadger_nodes_multiprocess_embedded_runtime_single_round() -> None:
    ...
    _assert_embedded_runtime_results(results)
...
def test_local_honeybadger_benchmark_rust_hosted_reports_subprotocol_timings() -> None:
    ...
    assert all(
        result.subprotocol_timings["tpke.encrypt.seconds"].total_seconds > 0.0
