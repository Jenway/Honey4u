...
def _build_mvba_predicate(
    params: DumboACSParams,
    proof_validity_cache: dict[PrbcProofValidationKey, bool],
) -> Callable[[bytes], bool]:
    def validate_proof(leader: int, proof: PrbcProof) -> bool:
        key = (leader, proof.roothash, proof.sigmas)
        cached = proof_validity_cache.get(key)
        if cached is not None:
            return cached

        valid = validate_prbc_proof(
    ...
    def _predicate(raw: bytes) -> bool:
        ...
        for leader, proof in enumerate(entries):
            if proof is None:
                continue
            if not validate_proof(leader, proof):
                continue
            valid += 1
...
async def dumbo_acs(
    ...
    def local_proof_if_available(leader: int, remote_proof: PrbcProof) -> PrbcProof | None:
        ...
        try:
            ...
            return None
        if outcome.proof.roothash != remote_proof.roothash:
            return None
        proof_validity_cache[(leader, outcome.proof.roothash, outcome.proof.sigmas)] = True
    ...
    try:
        ...
        while len(valid_senders) < params.N - params.f:
            ...
            if sender != message.leader:
                continue
            proof = local_proof_if_available(sender, message.proof) or message.proof
            if proof is message.proof and not validate_proof(sender, proof):
                continue
            valid_senders.add(sender)
            proofs[sender] = proof

        await mvba_input.put(_serialize_prbc_vector(tuple(proofs)))
        selected_proofs = _deserialize_prbc_vector(await mvba_output.get(), params.N)
        if sum(proof is not None for proof in selected_proofs) < params.N - params.f:
            raise ProtocolInvariantError("DumboACS selected fewer than N-f PRBC proofs")

        selected_leader_vector: list[int | None] = [None] * params.N
        selected_payload_vector: list[bytes | None] = [None] * params.N
        for leader, selected_proof in enumerate(selected_proofs):
            ...
            selected_leader_vector[leader] = leader
            selected_payload_vector[leader] = outcome.value

        decision: DumboACSDecision | tuple[bytes | None, ...]
        if output_mode == "payloads":
            decision = tuple(selected_payload_vector)
        else:
            decision = tuple(selected_leader_vector)
