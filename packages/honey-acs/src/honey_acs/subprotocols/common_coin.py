...
class SharedCoin:
    ...
    def _is_decided(self, round_id: int) -> bool:
        future = self._output.get(round_id)
        return future.done() if future is not None else False
    ...
    async def _recv_loop(self, receive_queue: asyncio.Queue) -> None:
        try:
            while True:
                ...
                # Once a coin round is decided, extra late shares cannot change the result.
                if self._is_decided(round_id):
                    continue

                if sender_id in self._received.get(round_id, {}):
                    continue
