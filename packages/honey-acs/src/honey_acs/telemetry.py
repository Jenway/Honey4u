...
def _normalize_timing_snapshot(
    ...
    aggregated: dict[str, dict[str, float | int]] = defaultdict(
        lambda: {"sample_count": 0, "total_seconds": 0.0, "max_seconds": 0.0}
    )
    for raw_name, raw_summary in snapshot_timings.items():
        metric_name = str(raw_name).split("{", 1)[0]
        target = aggregated[metric_name]
        target["sample_count"] = int(target["sample_count"]) + int(raw_summary.get("count", 0))
        target["total_seconds"] = float(target["total_seconds"]) + float(
            raw_summary.get("total", 0.0)
        )
        target["max_seconds"] = max(
            float(target["max_seconds"]),
            float(raw_summary.get("max", 0.0)),
        )
